...
class Canos_Widget_Module_IDs extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'canos_widget_module_ids',
			'description' => esc_html__( 'Your site\'s Posts filtered by IDs.', 'canos' )
		);
		parent::__construct( 'canos_widget_module_ids', esc_html__( 'Canos Post Modules - By IDs', 'canos' ), $widget_ops );
		$this->alt_option_name = 'canos_widget_module_ids';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'canos_widget_module_ids', 'widget' );
		}
		...
		$ids = ( ! empty( $instance[ 'ids' ] ) ) ? $instance[ 'ids' ] : false;
		if ( ! $ids ) {
			$ids = false;
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'canos_widget_module_ids', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'canos_widget_module_ids' ] ) ) {
			delete_option( 'canos_widget_module_ids' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'canos_widget_module_ids', 'widget' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'ids' ) ); ?>"><?php esc_html_e( 'ID(s):', 'canos' ); ?></label>
			...
			<br>
			<small><?php esc_html_e( 'Type a list of posts IDs separated by commas (eg. 1,5,8).', 'canos' ); ?></small>
		</p>
...
add_action( 'widgets_init', 'canos_widget_module_ids' );

function canos_widget_module_ids() {
	register_widget( 'Canos_Widget_Module_IDs' );
}
