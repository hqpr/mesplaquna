...
class Canos_Widget_Module_Recent extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'canos_widget_module_recent',
			'description' => esc_html__( 'Your site\'s most recent Posts.', 'canos' )
		);
		parent::__construct( 'canos_widget_module_recent', esc_html__( 'Canos Post Modules - By Date', 'canos' ), $widget_ops );
		$this->alt_option_name = 'canos_widget_module_recent';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'canos_widget_module_recent', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'canos_widget_module_recent', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'canos_widget_module_recent' ] ) ) {
			delete_option( 'canos_widget_module_recent' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'canos_widget_module_recent', 'widget' );
	}
...
add_action( 'widgets_init', 'canos_widget_module_recent' );

function canos_widget_module_recent() {
	register_widget( 'Canos_Widget_Module_Recent' );
}
