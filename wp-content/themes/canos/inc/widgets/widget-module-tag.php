<?php
/**
 * Post modules - Filter by tag
 *
 * @package Canos
 */

class Canos_Widget_Module_Tag extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'canos_widget_module_tag',
			'description' => esc_html__( 'Your site\'s Posts filtered by tag.', 'canos' )
		);
		parent::__construct( 'canos_widget_module_tag', esc_html__( 'Canos Post Modules - By Tag', 'canos' ), $widget_ops );
		$this->alt_option_name = 'canos_widget_module_tag';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'canos_widget_module_tag', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'canos_widget_module_tag', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'canos_widget_module_tag' ] ) ) {
			delete_option( 'canos_widget_module_tag' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'canos_widget_module_tag', 'widget' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tag' ) ); ?>"><?php esc_html_e( 'Tag ID(s):', 'canos' ); ?></label>
			...
			<br>
			<small><?php esc_html_e( 'Type a list of tag IDs separated by commas (eg. 1,5,8).', 'canos' ); ?></small>
		</p>
...
add_action( 'widgets_init', 'canos_widget_module_tag' );

function canos_widget_module_tag() {
	register_widget( 'Canos_Widget_Module_Tag' );
}
