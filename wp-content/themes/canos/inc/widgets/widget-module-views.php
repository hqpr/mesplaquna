...
class Canos_Widget_Module_Views extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'canos_widget_module_views',
			'description' => esc_html__( 'Your site\'s Posts ordered by views count.', 'canos' )
		);
		parent::__construct( 'canos_widget_module_views', esc_html__( 'Canos Post Modules - By Views', 'canos' ), $widget_ops );
		$this->alt_option_name = 'canos_widget_module_views';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'canos_widget_module_views', 'widget' );
		}
		...
		$r_args = array(
			...
			'meta_key'            => '_canos_framework_post_views_count',
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'canos_widget_module_views', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'canos_widget_module_views' ] ) ) {
			delete_option( 'canos_widget_module_views' );
		}
...
add_action( 'widgets_init', 'canos_widget_module_views' );

function canos_widget_module_views() {
	register_widget( 'Canos_Widget_Module_Views' );
}
